...
def getRecipe(mealVal, restrictions, allergiesVal, calories, method, time, ingredientsVal):
    ...
    api_key = "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"
    openai.api_key = api_key

    prompt = f"Create a recipe using some/all of these ingredients as you see fit: {', '.join(ingredients)}. The recipe should include detailed instructions. Here are the 'dietary_restrictions': {', '.join(dietaryRestrictions)}. Here is what the user is allergic to: {', '.join(allergies)}. Here are possible ways the user can cook the meal: {', '.join(cookingMethods)}. The max time limit for the meal is {timeLimit}. Here is the MAX number of calories the meal should be: {cals}... Give me my response as a JSON with the 'name', 'ingredients', 'instructions', 'nutritional_facts' as the keys. Under each ingredient include the 'portion', 'calories', 'protein', 'fat', 'carbs', and 'fibers'. Generate an array called 'dietary_restrictions' of all the dietary restrictions that the recipe follows"+'You should return an object in this format: { "name": "<name of the dish>", "ingredients": [ { "ingredient": "<Name of the ingredient>", "portion": "<Portion or measurement>", "calories": <Calories count>, "protein": <Protein count>, "fat": <Fat count>, "carbs": <Carbohydrate count>, "fibers": <Fiber count> }, ... // Repeated sections for other ingredients ], "instructions": [ "<Step 1 of cooking>", "<Step 2 of cooking>", ... // Repeated steps for all the cooking instructions ], "nutritional_facts": { "calories": <Total calories>, "protein": <Total protein>, "fat": <Total fat>, "carbs": <Total carbs>, "fibers": <Total fibers> }, "dietary_restrictions": ["<Dietary restriction 1>", "<Dietary restriction 2>", ...] }'
